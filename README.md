...
<div align="right"><b>中文</b> | <a href="README_EN.md">English</a></div>
<br clear="all">
<br>
<div align="center">
<p>
...
8. **整理校对结果** — 在 <kbd>列表</kbd> 视图中，点击条目以选中，<kbd>鼠标右键</kbd> 进行 `剔除` / `恢复` （按住 <kbd>鼠标左键</kbd> 拖动可 `批量选中`）；在 <kbd>卡片</kbd> 视图中，点击 `卡片` 进行 `剔除` / `恢复`
...
使用 `/add-language` 命令自动生成规则文件并注册到 [`rules/languages.json`](rules/languages.json)
...
> [!NOTE]
> 语种自动检测当前仅区分 `CJK（中日韩统一表意文字）` 与 `Latin` 字符。若新语种与现有语种使用同一书写系统，建议在 `向导` 中手动指定校对语种。
